...
const cardBox = (extra = {}) => ({
  ...
  borderRadius: "12px", padding: "20px", marginBottom: "20px", ...extra,
});
...
const ResultCard = ({ badge, title, children, reportData, reportType, onBuildReport }) => (
  <div className="fade-in" style={{
    ...
    <div style={{ marginTop: "16px", textAlign: "right" }}>
      <Btn onClick={() => onBuildReport(reportData, reportType)} color={C.violet} dimColor={C.violetDim} style={{ padding: "10px 22px", fontSize: "13px" }}>
...
function App() {
  ...
  const analyzeIOC = async () => {
    ...
    try {
      ...
      setIocResult(extracted);

      if (extracted?.riskScore > 70) {
        const alert = {
          id: Date.now(),
          ip: extracted.ips?.[0] || "IOC-Scan",
          riskScore: extracted.riskScore,
          severity: extracted.riskScore > 150 ? "CRITICAL" : "HIGH",
          message: "Suspicious IOC detected",
        };
        setAlerts(prev => {
  	const exists = prev.find(a => a.ip === alert.ip);
        ...
        <>
          ...
          {result ? (
            (() => {
              ...
              return (
                <ResultCard badge={badge} title="🔎 Analysis Result" reportData={result} reportType="ip" onBuildReport={buildReport}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                    ...
                    <div style={{ background: C.card, borderRadius: "8px", padding: "14px" }}>
                      ...
                      <p style={{ ...val, fontSize: "28px", color: badge.color }}>
              ...
              <div style={{ marginTop: "12px", background: C.card, padding: "14px", borderRadius: "8px", border: `1px solid ${C.border}`, borderLeft: `3px solid ${C.violet}` }}>
                {(() => {
                  const mitre = getMitreMapping(result.riskScore);
                  return (
                    <>
                      <p style={{ margin: "0 0 8px", color: C.violet, fontWeight: "bold", fontSize: "13px" }}>🛡️ MITRE ATT&amp;CK: {mitre.technique}</p>
                      <p style={{ fontSize: "13px", opacity: 0.7, margin: "4px 0" }}><b>Technical:</b> {mitre.tech}</p>
                      <p style={{ fontSize: "13px", opacity: 0.7, margin: "4px 0" }}><b>Simple:</b> {mitre.simple}</p>
                      <p style={{ margin: "10px 0 6px", fontSize: "13px", fontWeight: "bold" }}>🛠️ Remediation:</p>
                      <ul style={{ paddingLeft: "18px", margin: 0 }}>
                        {mitre.remediation.map((r, i) => (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {[...history].reverse().map(item => {
                  ...
                  const rowBg  = item.riskScore > 80 ? "#7f1d1d18" : "transparent";
                  return (
                    <tr key={item.id}
                      onMouseOver={e => e.currentTarget.style.background = C.card}
                      onMouseOut={e => e.currentTarget.style.background = rowBg}
                      style={{ background: rowBg, borderBottom: `1px solid ${C.border}`, transition: "background 0.2s" }}>
...
{/*  SOC REPORT DROPDOWN (PASTE RIGHT AFTER SCAN HISTORY) */}
<details style={{ marginTop: "20px" }}>
  <summary style={{ cursor: "pointer", fontWeight: "bold", fontSize: "16px" }}>
    📄 View SOC Threat Analysis Report
  </summary>
  ...
  <div style={{
    ...
    <p><b>Overview:</b> This Threat Intelligence Dashboard simulates a SOC workflow by analyzing indicators using multi-source intelligence.</p>

    <p><b>Detection Logic:</b></p>
    <ul>
      <li>Risk scoring based on abuse confidence, malware detections, and exposed services</li>
      <li>Threshold-based alert generation</li>
      <li>Deduplication to prevent alert fatigue</li>
    </ul>

    <p><b>MITRE ATT&CK Mapping:</b></p>
    <ul>
      <li>T1595 – Active Scanning</li>
      <li>T1046 – Service Discovery</li>
    </ul>

    <p><b>Features:</b></p>
    <ul>
      <li>IP, URL, and Hash analysis</li>
    ...
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr>
          ...
          <th style={{ textAlign: "center", padding: "10px" }}>Severity</th>
      ...
      <tbody>
        {alerts.map(a => (
          <tr key={a.id} style={{ borderBottom: "1px solid #2a2a2a" }}>
            ...
            <td style={{ padding: "10px", textAlign: "center", color: C.red }}>
              {a.severity}
            </td>
      ...
      {activeTab === "url" && (
        <>
          ...
          {urlResult && (() => {
            ...
            return (
              <ResultCard badge={badge} title="🔗 URL Analysis Result" reportData={urlResult} reportType="url" onBuildReport={buildReport}>
      ...
      {activeTab === "hash" && (
        <>
          ...
          {hashResult && (() => {
            ...
            return (
              <ResultCard badge={badge} title="🧬 Hash Analysis Result" reportData={hashResult} reportType="hash" onBuildReport={buildReport}>
                <div style={{ background: C.card, borderRadius: "8px", padding: "12px", marginBottom: "12px" }}>
      ...
      {activeTab === "ioc" && (
        <>
          ...
          {iocResult && (() => {
            ...
            return (
              <ResultCard badge={badge} title="📊 IOC Extraction Report" reportData={iocResult} reportType="ioc" onBuildReport={buildReport}>
                {/* Score row */}
